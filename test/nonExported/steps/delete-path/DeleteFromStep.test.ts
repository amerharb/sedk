...
describe('DeleteFromStep', () => {
	...
	it('returns: [FROM "table1"]', () => {
		const actual = new DeleteFromStep(builderData, rootStep, table1).getStepStatement()
		expect(actual).toEqual('FROM "table1"')
	})
	it('returns: [FROM "table1" AS "t1"]', () => {
		const actual = new DeleteFromStep(builderData, rootStep, table1.as('t1')).getStepStatement()
