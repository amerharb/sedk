...
describe('LimitStep', () => {
	...
	it('returns: [LIMIT 1]', () => {
		const actual = new LimitStep(builderData, rootStep, 1).getStepStatement()
		expect(actual).toEqual('LIMIT 1')
	})
	/** Postgres accept decimal value */
	it(`returns: [LIMIT 1.7]`, () => {
		const actual = new LimitStep(builderData, rootStep, 1.7).getStepStatement()
		expect(actual).toEqual(`LIMIT 1.7`)
	})
	/** Postgres accept null in limit */
	it(`returns: [LIMIT NULL]`, () => {
		const actual = new LimitStep(builderData, rootStep, null).getStepStatement()
		expect(actual).toEqual(`LIMIT NULL`)
	})
	it(`returns: [LIMIT ALL]`, () => {
		const actual = new LimitStep(builderData, rootStep, ALL).getStepStatement()
		expect(actual).toEqual(`LIMIT ALL`)
	})
	it(`returns: [LIMIT $1]`, () => {
		const actual = new LimitStep(builderData, rootStep, 1, true).getStepStatement()
		expect(actual).toEqual(`LIMIT $1`)
	})
