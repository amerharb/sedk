import { $, LogicalOperator, builder } from 'sedk-mysql'
...
describe('Test binder with multi builders', () => {
	...
	it('Produces [SELECT ? FROM `table1`;] 2 binds at the same time', () => {
		const actual1 = sql1
			.select($(1))
			.from(table1)
		const actual2 = sql2
		...
		expect(actual1.getSQL()).toEqual('SELECT ? FROM `table1`;')
		expect(actual1.getBindValues()).toEqual([1])
		expect(actual2.getSQL()).toEqual('SELECT ? FROM `table1`;')
