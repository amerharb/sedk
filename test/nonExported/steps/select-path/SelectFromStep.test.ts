...
describe('SelectFromStep', () => {
	...
	it('returns: [FROM "table1"]', () => {
		const actual = new SelectFromStep(builderData, rootStep, [table1]).getStepStatement()
		expect(actual).toEqual('FROM "table1"')
	})
	it('returns: [FROM "table1" AS "t1"]', () => {
		const actual = new SelectFromStep(builderData, rootStep, [table1.as('t1')]).getStepStatement()
		expect(actual).toEqual('FROM "table1" AS "t1"')
	})
	it('returns: [FROM "table1", "table2"]', () => {
		const actual = new SelectFromStep(builderData, rootStep, [table1, table2]).getStepStatement()
